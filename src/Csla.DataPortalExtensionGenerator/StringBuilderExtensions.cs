...
internal static class StringBuilderExtensions {
    ...
    public static StringBuilder AppendExcludeFromCodeCoverageJustification(this StringBuilder sb, GeneratorOptions options) {
        if (!IsJustificationSupported(options.Tfms)) {
            return sb;
        }

        return sb.Append("Justification = \"Generated by the Ossendorf.Csla.DataPortalExtensionsGenerators source generator.\"");

        static bool IsJustificationSupported(EquatableArray<string> tfms) {
            for (var i = 0; i < tfms.Count; i++) {
                var tfm = tfms[i];

                if (tfm.StartsWith("net47", StringComparison.OrdinalIgnoreCase) || tfm.StartsWith("net48", StringComparison.OrdinalIgnoreCase) ||
                    tfm.StartsWith("netstandard2", StringComparison.OrdinalIgnoreCase)) {
                    return false;
                }
            }
